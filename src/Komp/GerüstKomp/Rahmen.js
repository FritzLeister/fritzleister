...
export default function Rahmen({
    ...
    const renderWallPlaneBeam = (key, position, size, rotation, approxSize) => {
        ...
        return (
            <group key={key}>
                {oberfläche && (
                    <mesh position={position} rotation={rotation} geometry={getBoxGeometry(size)} material={surfaceMaterial} />
                )}
                {frame && (
                    <lineSegments position={position} rotation={rotation} geometry={getEdgesGeometry(size)} material={lineMaterial} />
                )}
            </group>
        )
    }
